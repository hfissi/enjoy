  ...
  save()
{
  ...
  if(this.id === undefined){
    this.commentService.addComment(this.comment).subscribe((data)=>this.router.navigateByUrl('/listc'));  
  }else{
    this.commentService.updateComment(this.id,this.comment).subscribe((data)=>this.router.navigateByUrl('/listc'));  
